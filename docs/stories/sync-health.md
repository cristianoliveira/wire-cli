...
### 17) Sync Health: `wire sync status` checks overall account readiness `[Planned] [Must]`
...
### 18) Sync Health: `wire sync status --verbose` displays detailed metrics `[Planned] [Must]`
...
Acceptance criteria:
- Given I run `wire sync status --verbose`, then I see detailed metrics: sync lag (ms since last sync), number of pending messages, MLS migration status (percentage complete or "done"), key-package count per device, and event-stream status.
...
- Given a specific issue is detected, the output includes a recovery suggestion (e.g., "Sync lag is high; try: `wire sync reset` or check your network").
...
### 19) Sync Health: `wire sync status --diagnose` performs targeted diagnostics `[Planned] [Should]`
...
Acceptance criteria:
- Given I run `wire sync status --diagnose`, then the CLI performs a series of checks and outputs a diagnosis report.
...
### 20) Sync Health: `wire sync reset` initiates a manual sync reset `[Planned] [Could]`
...
## Current CLI Contract (Sync Health)
