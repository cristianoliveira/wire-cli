...
### 13) Device Management: `wire device list` displays all active devices `[Planned] [Must]`
...
Acceptance criteria:
- Given I am authenticated, when I run `wire device list`, then I see a table/list of all active devices with columns: device ID, device type, fingerprint hash, and last activity timestamp.
...
- Given I am not authenticated or my session is invalid, when I run `wire device list`, then access is denied with a login prompt and non-zero exit.
...
### 14) Device Management: `wire device info <device-id>` displays device details `[Planned] [Should]`
...
### 15) Device Management: `wire device delete <device-id>` revokes a device `[Planned] [Must]`
...
### 16) Device Management: `wire device rename <device-id>` assigns a friendly name to a device `[Planned] [Could]`
...
- Non-existent device returns exit code `14` with guidance to run `wire device list`.
- Unauthorized/missing sessions return exit code `11`.

### `wire device delete <device-id>`
...
### `wire device rename <device-id> --name <label>`
